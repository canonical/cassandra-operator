...
def test_network_cut_without_ip_change(juju: jubilant.Juju, continuous_writes: ContinuousWrites, app_name: str) -> None:
    ...
    for attempt in Retrying(
        wait=wait_fixed(10), stop=stop_after_delay(180), reraise=True
    ):
        with attempt:
           for name, unit in juju.status().apps[app_name].units.items():
               if name == stoped_unit_name:
                   continue
               
               logger.info(f"Checking: {app_name}/{unit.machine} if node {stoped_unit_host} is down")
               assert not check_node_is_up(juju, app_name, int(unit.machine), stoped_unit_host), "node is still UN in cluster"

    new_leader = get_leader_unit(juju, app_name)

    assert stoped_unit_name != new_leader
    ...
    # check node is up and ip not chaged
    assert check_node_is_up(juju, app_name, int(stoped_unit_name.split("/")[1]), stoped_unit_host)
...
def test_network_cut(juju: jubilant.Juju, continuous_writes: ContinuousWrites, app_name: str) -> None:
    ...
    continuous_writes.start(juju, app_name, hosts=no_stoped_hosts, replication_factor=3, context_str="test_network_cut")
    ...
    for attempt in Retrying(
        ...
        with attempt:
            for name, unit in juju.status().apps[app_name].units.items():
                ...
                logger.info(f"Checking: {app_name}/{unit.machine} if node {stoped_unit_name}:{stoped_unit_host} is down")
                assert not check_node_is_up(juju, app_name, int(unit.machine), stoped_unit_host), "node is still UN in cluster"
    ...
    new_stoped_unit_host = get_hosts(juju, app_name, stoped_unit_name)[0]

    # check node is up and ip is changed
    assert new_stoped_unit_host != stoped_unit_host
    assert check_node_is_up(juju, app_name, int(stoped_unit_name.split("/")[1]), new_stoped_unit_host)    
    
    continuous_writes.stop_and_assert_writes([new_stoped_unit_host])
    #TODO: make shure writes stops after test failed
