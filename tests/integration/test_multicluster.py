...
def test_writes_not_replicated(juju: jubilant.Juju, app_name: str) -> None:
    ...
    for i, cluster in enumerate(fed):
        got_rows = read_n_rows(
            hosts=[
                get_unit_address(juju, cluster.app_name, get_leader_unit(juju, cluster.app_name))
            ],
            password=app_secret_extract(juju, cluster.app_name, OPERATOR_PASSWORD),
            ks=ks,
            table=tbl,
    ...
    got_0 = read_n_rows(
        hosts=[get_unit_address(juju, fed[0].app_name, get_leader_unit(juju, fed[0].app_name))],
    ...
    got_1 = read_n_rows(
        hosts=[get_unit_address(juju, fed[1].app_name, get_leader_unit(juju, fed[0].app_name))],
