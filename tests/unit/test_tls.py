...
from ops.testing import Context, PeerRelation, Relation, Secret, State, RawSecretRevisionContents
...
class DefaultSertificateContext:
    context: Context
...
@pytest.fixture
def ctx() -> Context:
    ctx = Context(CassandraCharm, unit_id=0)
    return ctx

def default_certificate_context(ctx: Context) -> DefaultSertificateContext:
    peer_relation = testing.PeerRelation(
        id=1,
    ...
    return DefaultSertificateContext(
        context=ctx,
...
def test_default_certificates_files_setup(ctx):
    """Test enabled default internal TLS of a new cluster. Files for internal TLS is created."""
...
def test_relation_created(
    ...
    """Check that relating the charm toggle TLS state in the databag."""
...
@pytest.mark.parametrize("is_leader", [True, False])
def test_tls_enabled_but_not_ready(ctx, is_leader) -> None:
...
@pytest.mark.parametrize("is_leader", [True, False])
def test_certificate_available_new_cluster(ctx, is_leader) -> None:
    """Test CertificateAvailable event when enabling TLS."""
