...
from src.common.literals import PEER_RELATION
...
def test_start_maintenance_status_when_starting():
    ...
    with (
        patch("workload.CassandraWorkload.write_file"),
...
def test_start_only_after_leader_active():
    ...
    with (
        ...
        patch("workload.CassandraWorkload.start"),
        patch("subprocess.run"),
        patch("managers.cluster.ClusterManager.start_node") as start_node,
    ):
        state_out = ctx.run(ctx.on.start(), state_in)
...
def test_config_changed_no_restart():
    ...
    with (
        patch("workload.CassandraWorkload.write_file"),
        patch("workload.CassandraWorkload.start"),
        patch("managers.cluster.ClusterManager.restart_node") as restart_node,
        patch("subprocess.run"),
    ):
        state_out = ctx.run(ctx.on.config_changed(), state_in)
        assert state_out.unit_status == ops.MaintenanceStatus("waiting for Cassandra to start")
        restart_node.assert_not_called()
